...
1. [CNI 插件的二进制执行文件的作用](https://cizixs.com/2017/05/23/container-network-cni/)

2. [CNI 插件基本使用方法](https://www.dasblinkenlichten.com/understanding-cni-container-networking-interface/)

3. [在 Docker 中使用 CNI 插件](https://www.dasblinkenlichten.com/using-cni-docker/)

4. [在 Kubernetes 中使用 CNI 插件](https://segmentfault.com/a/1190000017182169)

5. [CNI 基础插件和 flannel、calico 等的关系 -- CNI 插件能完成的事情就是建立容器到虚拟机上的网络。而要实现跨虚拟机的容器之间的网络，需要使用 flannel、calico 等组件](https://segmentfault.com/a/1190000017182169)

6. [使用 CNI 插件的 IPAM 和 DNS 管理功能](https://www.dasblinkenlichten.com/ipam-dns-cni/)

7. [CNI 插件的一般配置选项](https://github.com/containernetworking/cni/blob/master/SPEC.md)

8. [CNI 项目说明文档地址](https://github.com/containernetworking/cni)

9. [CNI 插件二进制文件说明文档地址](https://github.com/containernetworking/plugins)
...
If you’ve been paying attention to the discussions around container networking you’ve likely heard the acronym CNI being used.  CNI stands for Container Networking Interface and it’s goal is to create a generic plugin-based networking solution for containers.  CNI is defined by a spec (read it now, its not very long) that has some interesting language in it.  Here are a couple of points I found interesting during my first read through…  如果您一直关注有关容器网络的讨论，您可能已经听过使用CNI的首字母缩略词。 CNI代表容器网络接口，它的目标是为容器创建一个通用的基于插件的网络解决方案。 CNI由一个规范定义（现在读它，它不是很长），其中有一些有趣的语言。 以下是我在第一次阅读时发现有趣的几点......
...
2017/02/17 09:46:01 Error retriving last reserved ip: Failed to retrieve last reserved ip: open /var/lib/cni/networks/mybridge/last_reserved_ip: no such file or directory  检索上次保留的ip时出错：无法检索上次保留的ip：open /var/lib/cni/networks/mybridge/last_reserved_ip：没有这样的文件或目录
{
    "ip4": {
        "ip": "10.15.20.2/24",
        "gateway": "10.15.20.1",
