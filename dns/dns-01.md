...
CoreDNS is a single process, written in Go, that covers all of the functionality of the previous system. A single container resolves and caches DNS queries, responds to health checks, and provides metrics.  CoreDNS是一个用Go编写的单一进程，它涵盖了以前系统的所有功能。 单个容器解析并缓存DNS查询，响应运行状况检查并提供指标。
